...
def extract_niveau(sigle: str) -> int:
    """Extrait le niveau depuis le premier chiffre du sigle (ex: LOG210 → 2)."""
...
def main():
    ...
    canonical = [
        transform_course(c, hors_perimetre=False) for c in courses["PROGRAM"]
    ] + [
        transform_course(c, hors_perimetre=True)  for c in courses["OTHER"]
    ]

    # Dédoublonnage par sigle (priorité au premier trouvé = PROGRAM)
    seen, unique = set(), []
    for c in canonical:
